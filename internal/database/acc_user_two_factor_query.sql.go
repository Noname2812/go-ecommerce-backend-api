...
// versions:
//   sqlc v1.29.0
// source: acc_user_two_factor_query.sql
...
func (q *Queries) GetTwoFactorMethodByID(ctx context.Context, twoFactorID uint32) (AccUserTwoFactor, error) {
	row := q.db.QueryRowContext(ctx, getTwoFactorMethodByID, twoFactorID)
	var i AccUserTwoFactor
	err := row.Scan(
		&i.TwoFactorID,
		&i.UserID,
		&i.TwoFactorAuthType,
...
func (q *Queries) GetTwoFactorMethodByIDAndType(ctx context.Context, arg GetTwoFactorMethodByIDAndTypeParams) (AccUserTwoFactor, error) {
...
func (q *Queries) GetUserTwoFactorMethods(ctx context.Context, userID uint32) ([]AccUserTwoFactor, error) {
	rows, err := q.db.QueryContext(ctx, getUserTwoFactorMethods, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AccUserTwoFactor
	for rows.Next() {
		var i AccUserTwoFactor
		if err := rows.Scan(
			&i.TwoFactorID,
